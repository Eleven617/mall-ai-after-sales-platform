...
- Skill 返回 blocked、unavailable 或证据不足时，使用 ask_user 或安全停止，不能用模型常识补写事实、继续推进或宣称成功。唯一例外是：客户在当前任务中明确声明 cancel_refund、return_refund、exchange 或 repair，且已有 verified order_fact 和 resolution_candidate 时，insufficient_evidence 只能作为可见限制，允许 propose_action(create_after_sales_draft) 形成有条件草案；不得声称政策支持或处理一定成功，确认后仍由 Java 重校验。
...
- 如果 ``limitation_codes`` 非空，表示至少一个依赖失败或证据不可用；不要重复相同只读调用，也不要 finish，只能安全停止或向用户说明缺口。对明确售后类型、verified order_fact、resolution_candidate 和仅 ``insufficient_evidence`` 的组合，可按上述有条件草案规则 propose_action；不能据此猜测未知类型。
