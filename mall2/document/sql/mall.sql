...
-- ----------------------------
-- Table structure for ai_return_submission
-- ----------------------------
CREATE TABLE IF NOT EXISTS `ai_return_submission` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `member_id` bigint(20) NOT NULL COMMENT 'Java-authenticated member id',
  `idempotency_key` varchar(64) NOT NULL COMMENT 'server-generated AI confirmation key',
  `request_fingerprint` char(64) NOT NULL COMMENT 'SHA-256 of canonical confirmed request',
  `return_apply_id` bigint(20) NULL DEFAULT NULL COMMENT 'created oms_order_return_apply id',
  `create_time` datetime NOT NULL COMMENT 'claim time',
  `update_time` datetime NOT NULL COMMENT 'result bind time',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_member_idempotency_key` (`member_id`, `idempotency_key`),
  KEY `idx_return_apply_id` (`return_apply_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='AI售后提交幂等记录';

-- ----------------------------
-- Table structure for ai_after_sales_application
-- ----------------------------
-- New AI-created after-sales requests use a separate generic aggregate. The
-- legacy oms_order_return_apply table above stays unchanged for its original
-- administrator workflow and historical records.
CREATE TABLE IF NOT EXISTS `ai_after_sales_application` (
  ...
  `reviewed_by` varchar(64) NULL DEFAULT NULL COMMENT '授权审核人员用户名，仅内部审计使用',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='AI通用售后申请';
