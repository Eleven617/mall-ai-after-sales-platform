...
旧的 72/72、36/36、Grounding 和 122/122 Durable 报告绑定旧 Runtime，不能替代当前结果。当前分支 `mall-ci` 的 public-release 仍失败，`quality-evaluation` 成功；不能宣称远程 CI 全绿、生产 SLA、真实用户泛化、真实外部支付/仓储/物流/维修履约或模型成本。唯一事实源：[`current-release-facts.json`](../evidence/current-release-facts.json)。
